...
import library.mgiandia.com.androidlibrary.view.Publisher.PublisherDetails.PublisherDetailsActivity;

public class ManagePublishersActivity extends AppCompatActivity implements SearchView.OnQueryTextListener
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        item_list_view.setOnItemClickListener(new AdapterView.OnItemClickListener()
        {
            ...
            {
                Intent intent = new Intent(new_context, PublisherDetailsActivity.class);
                intent.putExtra("publisher_id", ((Publisher)parent.getItemAtPosition(position)).getID());
    ...
    {//we intentionally don't make this code more compact, so we get what's going on
        ...
        if(requestCode == 0)//if "add new user activity finished"
        {
            ...
            {
                clear_search_bar();
                Toast.makeText(ManagePublishersActivity.this, data.getStringExtra("message_to_toast"), Toast.LENGTH_LONG).show();
            }
        }
